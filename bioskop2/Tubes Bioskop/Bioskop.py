...
class MainPage(Frame):
    ...
    def __init__(self, master):
        ...
        self.title_label = Label(self.master, font=("Roboto Bold", 16), bg="white", fg="black",
                                 text="=============================================\nSelamat datang di Bioskop Kelompok 8\nSilahkan memilih"
                                      " Anda sebagai Customer atau Karyawan:\n=============================================").pack(anchor=CENTER, pady=80)
        self.cust_button = Button(self.master, font=("Roboto Bold", 16), text="Customers", bg="#2ecc71", fg="white",
                                  command=self.customer_window).pack(anchor=CENTER, pady='3')
        self.emp_button = Button(self.master, font=("Roboto Bold", 16), text="Karyawan", bg="#2ecc71", fg="white",
                                 command=self.employee_window).pack(anchor=CENTER, pady='3')
        self.quit = Button(self.master, font=("Roboto Bold", 12), text="Keluar", bg="black", fg='white',
                           command=self.master.quit).pack(anchor=CENTER, pady='3')
        self.frame.pack()
...
class Customer(MainPage):
    ...
    def __init__(self, master):
        ...
        self.title = Label(self.master, font=("Helvetica", 16), text='Masukkan Username dan Password', bg="white", fg="black")
        ...
        self.checkbox = Checkbutton(self.master, text="Simpan Informasi Log-in")
        self.checkbox.grid(row=3, column=0, columnspan=2, pady='3')
        self.loginbutton = Button(self.master, text="Login", bg="#2ecc71", fg="white", command=self.login).grid(row=4, column=1, pady='3')
        self.back = Button(self.master, text="Kembali", bg="#2ecc71", fg="white", command=self.back).grid(row=4, column=0, pady='3')
        self.createnewaccount = Button(self.master, text="Buat Akun Baru", bg="black", fg="white", command=self.create).grid(row=5,
                                                                                                                  column=0,
                                                                                                                  columnspan=2,
                                                                                                                  pady='3')
    ...
    def login(self):
        ...
        if login1:
            ...
            self.master.withdraw()
            tkinter.messagebox.showinfo("---COMPLETE---", "Kamu telah berhasil login.", icon="info")
        else:
            tkinter.messagebox.showinfo("---ERROR---", "Masukkan informasi yang benar!", icon="warning")
...
class CustMain(Customer):
    '''Menu Utama Customer. User dapat mengganti profile, melihat riwayat pemesanan, serta mencari film berdasarkan tanggal dan waktu'''
    ...
    def __init__(self, master):
        ...
        self.label.place(x=0, y=0, relwidth=1, relheight=1)
        self.frame.pack()
        self.up = Button(self.master, font=("Helvetica", 16), text="Ganti Profile", bg="#2ecc71",
                         command=self.Update_Profile_window).pack(pady='5')
        self.bh = Button(self.master, font=("Helvetica", 16), text="Riwayat Pemesanan", bg="#2ecc71",
                         command=self.Booking_History_window).pack(pady='5')
        self.search = Label(self.master, font=("Helvetica", 16), text='Telusuri film berdasarkan tanggal:').pack()
        self.defdate1 = StringVar(self.master)
        self.defdate1.set("Senin 06/12/21")  # default value
        self.datelist1 = OptionMenu(master, self.defdate1, "Senin 06/12/21", "Selasa 07/12/21", "Rabu 08/12/21",
                                    "Kamis 09/12/21", "Jumat 10/12/21", "Sabtu 11/12/21", "Minggu 12/12/21")
        self.datelist1.pack()
        self.searching = Button(self.master, text="Telusuri", command=self.sch).pack()
        self.log = Button(self.master, font=("Helvetica", 12), text="Keluar", fg="white", bg='black',
    ...
    def logout(self):
        msg = tkinter.messagebox.askyesno('Keluar', 'Anda yakin akan keluar?')
        if msg:
            self.newWindow = Toplevel(self.master)
            self.newWindow.geometry('1096x720')
            self.app = MainPage(self.newWindow)
...
class SearchResults(CustMain):
    def __init__(self, master, output, search_date):
        ...
        self.label.place(x=0, y=0, relwidth=1, relheight=1)
        self.frame.grid(row=0, column=0)
        self.date = Label(self.master, text='Tanggal', font=("Helvetica", 16), width=8).grid(row=0, column=1, pady='1')
        self.time = Label(self.master, text='Waktu', font=("Helvetica", 16), width=6).grid(row=0, column=2, pady='1')
        self.title = Label(self.master, text='Judul', font=("Helvetica", 16), width=15).grid(row=0, column=3, pady='1')
        self.description = Label(self.master, text='Deskripsi', font=("Helvetica", 16), width=30).grid(row=0,
                                                                                                         column=4,
                                                                                                         pady='1')
        self.booked = Label(self.master, text='Dipesan', font=("Helvetica", 16), width=8).grid(row=0, column=5, pady='1')
        self.available = Label(self.master, text='Tersedia', font=("Helvetica", 16), width=8).grid(row=0, column=6,
                                                                                                    pady='1')
        self.book = Label(self.master, text='Pesan Tiket', font=("Helvetica", 16), width=12).grid(row=0, column=7,
                                                                                                  pady='1')
        ...
        for i in self.output:
            ...
            d = Button(self.master, text='Pesan', command=partial(self.boo, self.taken, self.datetime),
                       font=("Helvetica", 7))
            d.grid(row=output.index(i) + 1, column=7, pady='1')
        self.back = Button(self.master, font=("Helvetica", 12), text="Kembali", bg="black", fg="white",
                           command=self.back).grid(row=10, column=1, pady='1')
        self.log = Button(self.master, font=("Helvetica", 12), text="Keluar", fg="white", bg='black',
                          command=self.logout).grid(row=10, column=2, pady='1')

    def boo(self, gone, datetime7):
        msg = tkinter.messagebox.askyesno('Pesan', 'Anda yakin akan memesan tiket ini?')
        if msg:
            self.gone = gone
            ...
            alr = c3.fetchone()
            if alr:
                tkinter.messagebox.showinfo("---- ERROR ----", "Kamu sudah pernah memesan tiket film ini", icon="warning")
            elif self.gone == 100:
                tkinter.messagebox.showinfo("---- ERROR ----", "Film ini telah full dipesan", icon="warning")
            else:
                ...
                if td_year < 2000 or \
                        td_year == 2000 and td_month > 1 or \
                        td_year == 2000 and td_month == 1 and td_day > new_date or \
                        td_year == 2000 and td_month == 1 and td_day == new_date and td_hour >= new_time:
                    tkinter.messagebox.showinfo("---- ERROR ----", "Tanggal dan waktu telah terlewat!",
                                                icon="warning")
                else:
                    with conn2:
...
class CustProfile(CustMain):
    '''Customer dapat mengupdate profile'''
    ...
    def __init__(self, master):
        ...
        self.title = Label(self.master, text="Ubah data anda di bawah:", font=("Helvetica", 16)).grid(row=0,
                                                                                                               column=0,
                                                                                                               columnspan=2,
                                                                                                               pady='3')

        self.output = c.fetchone()
        self.first = Label(self.master, text="Nama depan:", width='15').grid(row=1, column=0, pady='3')
        ...
        self.last = Label(self.master, text="Nama Belakang:", width='15').grid(row=2, column=0, pady='3')
        self.lastname = Entry(self.master, width='30')
        self.lastname.insert(END, self.output[1])
        self.lastname.grid(row=2, column=1, pady='3')

        self.email = Label(self.master, text="Alamat E-mail:", width='15').grid(row=3, column=0, pady='3')
        self.emailadd = Entry(self.master, width='30')
        self.emailadd.insert(END, self.output[2])
        self.emailadd.grid(row=3, column=1, pady='3')

        self.ag = Label(self.master, text="Umur:", width='15').grid(row=4, column=0, pady='3')
        ...
        self.ps2 = Label(self.master, text="Konfirmasi Password:", width='15').grid(row=6, column=0, pady='3')
        ...
        self.update = Button(self.master, text="Ubah Detail", command=self.change).grid(row=7, columnspan=2,
                                                                                           pady='3')

        self.back = Button(self.master, font=("Helvetica", 12), text="Kembali", fg="white", bg='black',
                           command=self.back).grid(row=8, column=0, pady='3')
        self.log = Button(self.master, font=("Helvetica", 12), text="Keluar", fg="white", bg='black',
                          command=self.logout).grid(row=8, column=1, pady='3')

    def change(self):
        msg = tkinter.messagebox.askyesno('Ubah Profil', 'Anda yakin untuk merubah?')
        if msg:
            ...
            try:
                ...
                tkinter.messagebox.showinfo("---- ERROR ----", "Tolong isi semua kolom.", icon="warning")
            except InvalidEmailError:
                tkinter.messagebox.showinfo("---- ERROR ----", "Format E-Mail salah!", icon="warning")
            except PasswordsDoNotMatchError:
                tkinter.messagebox.showinfo("---- ERROR ----", "Password tidak sesuai!", icon="warning")
            except ValueError:
                tkinter.messagebox.showinfo("---- ERROR ----", "Format umur tidak sesuai!", icon="warning")
            else:
                with conn:
                ...
                self.app = CustProfile(self.newWindow)
                self.master.withdraw()
                tkinter.messagebox.showinfo("---- SUCCESSFUL ----", "Profil sukses diupdate.",
                                            icon="info")
...
class NewAccount(Customer):
    def __init__(self, master):
        ...
        self.title = Label(self.master, text="Isikan data Anda di bawah:", font=("Helvetica", 16)).grid(row=0,
                                                                                                              column=0,
                                                                                                              columnspan=2,
                                                                                                              pady='3')
        ...
        self.newpasswd2 = Label(self.master, text="Konfirmasi Password:", width='15').grid(row=7, column=0, pady='3')
        self.newpassword2 = Entry(self.master, width='30', show="*")
        self.newpassword2.grid(row=7, column=1, pady='3')

        self.update = Button(self.master, text="Buat Akun Baru", command=self.change).grid(row=8, columnspan=2,
                                                                                               pady='3')
    ...
    def change(self):
        ...
        try:
            ...
            tkinter.messagebox.showinfo("---- ERROR ----", "Username telah dipakai!", icon="warning")
        except PasswordsDoNotMatchError:
            tkinter.messagebox.showinfo("---- ERROR ----", "Password tidak sesuai!", icon="warning")
        except ValueError:
            tkinter.messagebox.showinfo("---- ERROR ----", "Format umur tidak sesuai!", icon="warning")
            ...
            self.app = Customer(self.newWindow)
            self.master.withdraw()
            tkinter.messagebox.showinfo("---- SUCCESSFUL ----", "Akun berhasil dibuat.",
                                        icon="info")
...
class Booked(CustProfile, SearchResults):
    def __init__(self, master, datetime1):
        ...
        self.heading = Label(self.master, text='Pemesanan berhasil! berikut detail pemesanan Anda:', width=45,
        ...
        self.da = Label(self.master, text="Tanggal:", width=15).grid(row=5, column=0, pady='3')
        self.date = Label(self.master, text=self.datetime1[0], width=30)
        self.date.grid(row=5, column=1, pady='3')

        self.ti = Label(self.master, text="Waktu:", width=15).grid(row=6, column=0, pady='3')
        ...
        self.se = Label(self.master, text="Nomor Kursi:", width=15).grid(row=7, column=0, pady='1')
...
class BookHist(Booked):
    def __init__(self, master):
        ...
        self.date = Label(self.master, text='Tanggal', font=("Helvetica", 16), width='15').grid(row=0, column=0)
        self.time = Label(self.master, text='Waktu', font=("Helvetica", 16), width='10').grid(row=0, column=1)
        self.title = Label(self.master, text='Judul', font=("Helvetica", 16), width='33').grid(row=0, column=2)
        self.seatno = Label(self.master, text='Nomor Kursi', font=("Helvetica", 16), width='15').grid(row=0, column=3)
        self.remo = Label(self.master, text='Hapus Pemesanan', font=("Helvetica", 16), width='15').grid(row=0, column=4)

        c3.execute('''SELECT date, time, seatno FROM bookings WHERE
                    username = ? ORDER BY date='Senin 06/12/21' DESC,
                                                    date='Selasa 07/12/21' DESC,
                                                    date='Rabu 08/12/21' DESC,
                                                    date='Kamis 09/12/21' DESC,
                                                    date='Jumat 10/12/21' DESC,
                                                    date='Sabtu 11/12/21' DESC,
                                                    date='Minggu 12/12/21' DESC,
                                                    time='1pm' DESC,
                                                    time='2pm' DESC,
                                                    time='3pm' DESC,
                                                    time='4pm' DESC,
        ...
        for i in tempresult:
            ...
            b3 = Button(self.master, text='hapus', font=("Helvetica", 12), width='15',
    ...
    def remove(self, date1, time):
        msg = tkinter.messagebox.askyesno('Hapus', 'Anda yakin untuk menghapus pemesanan?')
        if msg:
            td_hour = datetime.today().hour - 12
            td_day = date.today().day
            td_month = date.today().month
            ...
            if td_year > 2019 or \
                ...
                tkinter.messagebox.showinfo("---- ERROR ----", "Tanggal dan Waktu telah terlewat!", icon="warning")
            else:
                with conn3:
                ...
                tkinter.messagebox.showinfo("---- REMOVED ----", "Film dihapus dari riwayat pemesanan.", icon="info")
                self.newWindow = Toplevel(self.master)
                self.newWindow.geometry('1096x720')
                self.app = BookHist(self.newWindow)
                self.master.withdraw()
    ...
    def login(self):
        ...
        for i in lines:
            ...
            if username2 == j[0] and password2 == j[1]:
                ...
                self.master.withdraw()
                tkinter.messagebox.showinfo("---- SUCCESS ----", "Anda telah berhasil Login.", icon="info")
                break
        else:
            tkinter.messagebox.showinfo("---- ERROR ----", "Login gagal! Silahkan mencoba kembali", icon="warning")


class EmployeeMain(Employee):
    '''Menu utama karyawan. karyawan dapat melihat dan merubah daftar film'''
    ...
    def __init__(self, master):
        ...
        self.label.place(x=0, y=0, relwidth=1, relheight=1)
        self.frame.pack()
        self.showings = Button(self.master, font=("Helvetica", 16), text="Lihat daftar film", bg="#2ecc71",
                               command=self.See_list).pack(pady='5')
        self.add = Button(self.master, font=("Helvetica", 16), text="Tambahkan daftar film", bg="#2ecc71",
...
class Showings(EmployeeMain):
    '''Menunjukkan daftar tanggal dan waktu penayangan film. Tanggal mulai Senin 06/12/21 hingga Minggu 12/12/21, dan rentang waktu mulai 1-11pm (dengan penayangan terakhir pukul: 10pm).'''
    ...
    def __init__(self, master):
        ...
        self.date = Label(self.master, text='Tanggal', font=("Helvetica", 16), width=8).grid(row=0, column=1, pady='1')
        self.time = Label(self.master, text='Waktu', font=("Helvetica", 16), width=6).grid(row=0, column=2, pady='1')
        self.title = Label(self.master, text='Judul', font=("Helvetica", 16), width=18).grid(row=0, column=3, pady='1')
        self.description = Label(self.master, text='Deskripsi', font=("Helvetica", 16), width=32).grid(row=0,
                                                                                                         column=4,
                                                                                                         pady='1')
        self.booked = Label(self.master, text='Dipesan', font=("Helvetica", 16), width=8).grid(row=0, column=5, pady='1')
        self.available = Label(self.master, text='Tersedia', font=("Helvetica", 16), width=8).grid(row=0, column=6,
                                                                                                    pady='1')
        self.seeseats = Label(self.master, text='Kursi', font=("Helvetica", 16), width=8).grid(row=0, column=7,
                                                                                                 pady='1')
        c2.execute('''SELECT * FROM movies ORDER BY date='Senin 06/12/21' DESC,
                                                    date='Selasa 07/12/21' DESC,
                                                    date='Rabu 08/12/21' DESC,
                                                    date='Kamis 09/12/21' DESC,
                                                    date='Jumat 10/12/21' DESC,
        ...
        for i in output:
            ...
            self.taken = i[4]
            # self.taken = c2.fetchone()[0]  # returns the number of available seats for that movie
            d = Button(self.master, text='Lihat Kursi', command=partial(self.seats, self.taken), font=("Helvetica", 7),
                       width=8)
            ...
            d.grid(row=output.index(i) + 1, column=7, pady='1')
        self.exp = Button(self.master, font=("Helvetica", 12), text="Ekspor film ke text", bg="#2ecc71",
                          command=self.export).grid(row=35, column=3, pady='1')
        self.back = Button(self.master, font=("Helvetica", 12), text="Kembali", bg="black", fg="white",
                           command=self.back).grid(row=35, column=1, pady='1')
        self.log = Button(self.master, font=("Helvetica", 12), text="Keluar", bg="black", fg="white",
    ...
    def export(self):
        try:
            ...
            print('Di bawah ini adalah daftar tanggal dan waktu penayangan film.', file=file)
            c2.execute('''SELECT * FROM movies ORDER BY date='Senin 06/12/21' DESC,
                                                    date='Selasa 07/12/21' DESC,
                                                    date='Rabu 08/12/21' DESC,
                                                    date='Kamis 09/12/21' DESC,
            ...
            tkinter.messagebox.showinfo("---- SUCCESSFUL ----", "Film berhasil diekspor.",
                                        icon="info")
        except:
            tkinter.messagebox.showinfo("---- ERROR ----", "Terjadi kesalahan.",
                                        icon="warning")
...
class AddShowings(Showings):
    def __init__(self, master):
        ...
        self.title = Label(self.master, text="Silahkan tambahkan jadwal penayangan",
        ...
        self.defdate.set("Senin 06/12/21")  # default value
        self.datelist = OptionMenu(master, self.defdate, "Senin 06/12/21", "Selasa 07/12/21", "Rabu 08/12/21",
                                   "Kamis 09/12/21", "Jumat 10/12/21", "Sabtu 11/12/21", "Minggu 12/12/21")
        # self.datelist.config(width=30)
        self.datelist.grid(row=1, column=1, pady='3')

        self.time = Label(self.master, text="Waktu:", width=15).grid(row=2, column=0, pady='3')
        ...
        self.titl = Label(self.master, text="Judul:", width=15).grid(row=3, column=0, pady='3')
        self.title = Entry(self.master, width='45')
        self.title.grid(row=3, column=1, pady='3')

        self.des = Label(self.master, text="Deskripsi:", width=15).grid(row=4, column=0, pady='3')
        self.description = Entry(self.master, width='45')
        self.description.grid(row=4, column=1, columnspan=4, pady='3')

        self.addfilm = Button(self.master, text="Tambahkan penayangan film", command=self.addnew).grid(row=5, columnspan=5,
                                                                                              pady='3')
    ...
    def addnew(self):
        msg = tkinter.messagebox.askyesno('Keluar', 'Anda yakin dengan penambahan yang dilakukan?')
        if msg:
            ...
            if not new_date or not new_time or not new_title or not new_description:
                tkinter.messagebox.showinfo("---- ERROR ----", "Lengkapi data yang dibutuhkan.",
                                            icon="warning")
            else:
                if not c2.fetchall():
                    with conn2:
                        ...
                        tkinter.messagebox.showinfo("---- SUCCESSFUL ----",
                                                    "Penayangan berhasil ditambahkan!",
                                                    icon="info")
                else:
                    tkinter.messagebox.showinfo("---- ERROR ----", "Terjadi kesalahan!",
                                                icon="warning")
...
class SeeSeatsEmp(Showings):
    def __init__(self, master, booked):
        ...
        self.label.place(x=0, y=0, relwidth=1, relheight=1)
        self.frame.grid(row=0, column=0)
        self.title = Label(self.master, text='Daftar Kursi:    Hijau = Tersedia,'
                                             ' Merah = Tidak tersedia', font=("Helvetica", 16)).grid(row=0, columnspan=94)
        tens = self.booked // 10
        rem = self.booked % 10
        for i in range(tens):
            for j in range(5):
        ...
        self.back = Button(self.master, text="Kembali", font=("Helvetica", 12), fg="white", bg='black',
